...
import { TLogLevel } from './log-level'
 ...
 */
export class Logger {
  static readonly trace = Logger.getPrintFn(TLogLevel.Trace)
  static readonly debug = Logger.getPrintFn(TLogLevel.Debug)
  static readonly info = Logger.getPrintFn(TLogLevel.Info)
  static readonly warning = Logger.getPrintFn(TLogLevel.Warning)
  static readonly error = Logger.getPrintFn(TLogLevel.Error)

  /**
   * Select print function by logging level.
   * Logging level is defined at 'environment.ts'.
   */
  private static getPrintFn (logLevel: TLogLevel): (...optionalParams: any[]) => void {
  ...
  private static printBase (
    ...
    // Input parameter list, if the list length is larger than 1.
      ...
      if (typeof optionalParams[0][0] === 'string') {
        const tmpText = optionalParams[0][0]

...
export class ShiromusumeListComponent implements OnInit {
  ...
  getCharacterTypes (): void {
    Logger.trace()
    const obs = this.dataService.getCollection('CharacterTypes')
    ...
    if (obs !== undefined) {
      (obs as Observable<CharacterTypeDoc[]>).subscribe((x) => {
        ...
        const tmpObj = Object.assign({}, this.isCollectionLoadedMap)
  ...
  getCharacters (): void {
    ...
    const obs = this.dataService.getCollection('Characters')
